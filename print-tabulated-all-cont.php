...
<thead>
					...
					<?php 
					 ...
					 $t =  _sqlRetrieve_param("sum(rank),group_id","score","event_id",$_SESSION['eid'],"GROUP BY group_id order by sum(rank)");
					 }else{
						$t = _sqlRetrieve_param("sum(rank),con_id","score","event_id",$_SESSION['eid'],"GROUP BY con_id order by sum(rank)");
					 }
					  $tie = array();
					  $count = 1;
					   foreach($t as $d):
