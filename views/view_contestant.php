...
$sql = "SELECT count(con_id) as id from individual_contestant where event_id = '".$_SESSION['eid']."'";
  ...
  <script>
	...
	function request_page(pn){
		...
		var data = new XMLHttpRequest();
					data.open("POST", "views/getCon.php", true);
					...
					data.onreadystatechange = function() {
						...
						if (data.readyState == 4 && data.status == 200) {
                          ...
                          '<td>   <div class="user-panel">'+
							...
							'</div></td>'+
						 '<td>'+name+'</td>'+
                          '<td><div class="sparkbar" data-color="#00a65a" data-height="20">'+country+'</div></td>'+
						  '<td>'+btn+
                      ...
                      '</div></td>'+
						...
						sample.innerHTML = samp;
							...
							//modal for edit event
								up+='<form method="post" action="controller.php?action=update_contestant" enctype="multipart/form-data"><input type="hidden" name="con_id" id="con_id" value="'+id+'"/><div class="modal fade" id="edit'+id+'">'
							+'<div class="modal-dialog modal-md">'+
								'<div class="modal-content">'+
								  ...
								  '</div>'+
								  '<div class="modal-body">'+
									'<div class="box-body"><div class="row"><div class="col-md-6"><label for="exampleInputFile">Image</label><div class="img-square"><img width="100%" height="450px" id="img" src="upload_image/'+image+'" alt="Image preview..."></div></div><div class="col-md-6"><div class="form-group"><label for="exampleInputCandid">Candidate No.</label><input type="number" class="form-control" id="candid_no" placeholder="Candidate No." value="'+con_id+'" min="1" max="100" name="candid_no" required></div><div class="form-group"><label for="exampleInputDate">Last Name</label><input type="text" class="form-control" id="candid_lname" placeholder="Last Name" value="'+lname+'" name="candid_lname" required></div><div class="form-group"><label for="exampleInputDate">First Name</label><input type="text" class="form-control" id="candid_fname" placeholder=" First name" name="candid_fname" value="'+fname+'" required></div><div class="form-group"><label for="exampleInputDate">Middle Initial</label><input type="text" class="form-control" id="candid_mi" placeholder="Middle Initial" name="candid_mi" size="1" maxlength="1" value="'+mi+'" required></div> <div class="form-group"><label>Municipality/City</label><select class="form-control select2" data-placeholder="Select a municipality/city" style="width: 100%;" name="candid_address" id="candid_address"><?php $crit = _sqlSelect("city");foreach($crit as $value):?><option><?php echo $value['City_name'];?></option><?php endforeach;?></select></div>					<div class="form-group"><label for="venue">Barangay</label><input type="text" class="form-control" id="candid_barangay" placeholder="Barangay" value="'+barangay+'" name="candid_barangay" value=""><div class="standard" style=""><br /><input type="file"  id="candid_image" name="candid_image" value ="'+image+'" accept="image/*"></div></div></div></div></div></div>'+
								  '</div>'+
								  '<div class="modal-footer">'+
	...
	function search(pn){
		...
		var updatemodal = _("update-con");
		var data = new XMLHttpRequest();
					data.open("POST", "views/getSearchcon.php", true);
					data.setRequestHeader("Content-type","application/x-www-form-urlencoded");
					data.onreadystatechange = function() {
		...
		var data = new XMLHttpRequest();
				data.open("POST", "controller.php?action=update_con", true);
				data.setRequestHeader("Content-type","application/x-www-form-urlencoded");
				data.onreadystatechange = function() {
	...
	<script>
 	function delete_event(id){
		...
		var data = new XMLHttpRequest();
					data.open("POST", "views/delete_con.php", true);
					data.setRequestHeader("Content-type","application/x-www-form-urlencoded");
					data.onreadystatechange = function() {
						...
						if (data.readyState == 4 && data.status == 200) {
							...
							self.location = "controller.php?action=view_contestant";
	 ...
	 <?php include_once('views/tags/header.php');?>
      ...
      <div class="content-wrapper">
        ...
        <section class="content">
		 <?php
		 $ret = _sqlRetrieve_num("count(con_id) as id","individual_contestant","event_id",$_SESSION['eid']);
		 $ret = mysqli_fetch_assoc($ret);
		 if($ret['id'] == 0):?>
		            
                  		<div class="col-md-12 col-sm-2 col-md-offset-1 box0 text-center" style="margin:0 auto;">
						<a href="controller.php?action=add_contestant"><span class="li_heart">
                  			<div class="box1">
